...
// problem link -> https://leetcode.com/problems/01-matrix/m
...
vector<vector<int>> solve(vector<vector<int>>& mat){
    ...
    for(int i=0;i<n;i++){
        for(int j=0;j<m;j++){
            if(mat[i][j]==0){q.push(make_pair(i,j));distance[i][j]=0;visited[i][j]=1;}
     ...
     while(!q.empty()){
         ...
         for(auto p:dir){
             ...
             if(isvalid(new_x,new_y,n,m) and  !visited[new_x][new_y]){
