...
vector<vector<ll>>multiply(vector<vector<ll>>&mat1,vector<vector<ll>>& mat2,ll sz){
    vector<vector<ll>>mat3(sz,vector<ll>(sz,inf));

    for(int i=0;i<sz;i++){
        for(int j=0;j<sz;j++){
           
            for(int k=0;k<sz;k++){
                mat3[i][j]= min(mat3[i][j],(mat1[i][k]+mat2[k][j]));
            }
        }
    }
    return mat3;
        ...
        //if(n==0){cout<<0<<endl;return 0;}
         ...
         for(int i=0;i<m;i++){
           ...
           // mat[y][x]=1;
                ...
                vector<vector<ll>>temp=matrixExpoentiation(mat,k,n);



              if(temp[0][n-1]==inf){cout<<-1<<endl;return 0;}
